...
// Copyright (c) 2011-2017 The Mkwaycoin developers
...
static const int COINBASE_MATURITY_MKW = 30;
...
static const uint256 hashGenesisBlockOfficial("0x00000426184f1a0006058a165366a6f24b841861bed312132e571aa2aae38a26");
static const uint256 hashGenesisBlockTestNet("0x00001ffce01c611524dfb76103bf2766651719c3a049d7eb22a6a68b0aada9f5");
...
public:
    ...
    {
        // mkcoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // mkcoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // mkcoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // mkcoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // mkcoin: whether transaction is a coinstake
    bool fCoinStake;

    // mkcoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // mkcoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // mkcoin
            undo.nTime = nTime;            // mkcoin
        }
        return true;
    }
...
public:
    ...
    // mkcoin: block signature - signed by coin base txout[0]'s owner
    ...
    // mkcoin: two types of block: proof-of-work or proof-of-stake
    ...
    // mkcoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // mkcoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // mkcoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (mkcoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // mkcoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // mkcoin: proof-of-stake related block index fields
    unsigned int nFlags;  // mkcoin: block index flags
    enum
    {
