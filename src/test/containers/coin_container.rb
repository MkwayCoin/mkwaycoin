...
class CoinContainer
  def create(options = {})
    ...
    connects = links.map do |linked_name, alias_name|
      ...
      "#{connect_cmd}=$#{upname}_PORT_49523_TCP_ADDR:$#{upname}_PORT_49523_TCP_PORT"
    end
    ...
    bash_cmd += "./mkwaycoind " + cmd_args.join(" ")

    if options[:remove_addr_after_shutdown]
      bash_cmd += "; rm -f /root/.mkwaycoin/testnet/peers.dat"
    end

    if options[:remove_wallet_after_shutdown]
      bash_cmd += "; rm /root/.mkwaycoin/testnet/wallet.dat"
    end
  ...
  def load_data
    ...
    port = ports["49624/tcp"].first["HostPort"].to_i
    @rpc_port = port
    @port= ports["49523/tcp"].first["HostPort"].to_i
  end
