...
// Copyright (c) 2011-2015 The Mkwaycoin developers
...
int nCoinbaseMaturity = COINBASE_MATURITY_MKW;
...
const string strMessageMagic = "Mkwaycoin Signed Message:\n";
...
void SyncWithWallets(const uint256 &hash, const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // mkcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // mkcoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // mkcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // mkcoin: subsidy is cut in half every 16x multiply of difficulty
...
// mkcoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// mkcoin: find last block index up to pindex
...
#endif
{
    ...
    // mkcoin: target change every block
    // mkcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // mkcoin: should not enter safe mode for longer invalid chain
}
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // mkcoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // mkcoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // mkcoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (undo.nHeight != 0) {
                    ...
                    coins.fCoinStake = undo.fCoinStake; // mkcoin
                    coins.nTime = undo.nTime;           // mkcoin
                } else {
                    if (coins.IsPruned())
    ...
    // mkcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // mkcoin: coinbase reward check relocated to CheckBlock()
    ...
    // mkcoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // mkcoin: fees are not collected by miners as in bitcoin
    // mkcoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// mkcoin: total coin age spent in transaction, in the unit of coin-days.
...
// mkcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    // mkcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // mkcoin: record proof-of-stake hash value
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // mkcoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return state.DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // mkcoin: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // mkcoin: check that the block satisfies synchronized checkpoint
    ...
    // mkcoin: check pending sync-checkpoint
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    // mkcoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
    ...
    // mkcoin: ask for pending sync-checkpoint if any
    ...
    if (pblock->hashPrevBlock != 0 && !mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // mkcoin: getblocks may not obtain the ancestor block rejected
    ...
    // mkcoin: if responsible for sync-checkpoint send it
...
// mkcoin: entropy bit for stake modifier if chosen by modifier
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        // mkcoin: calculate stake modifier checksum
    ...
    // mkcoin: load hashSyncCheckpoint
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Mkwaycoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    fTxIndex = true; // mkcoin: txindex is always enabled
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "January 30, 2021 is the 30th day of the year 2021 in the Gregorian calendar";
        CTransaction txNew;
        txNew.nTime = 1612029090;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(1) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        block.nVersion = 1;
        block.nTime    = 1612029127;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2868161;

        if (fTestNet)
        {
            block.nTime    = 1612029110;
            block.nNonce   = 386577;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2c785e5dc576687575b9f1009ac36e04984e99604f87ee7c1106f46e0970f89a"));
        block.print();
        assert(hash == hashGenesisBlock);
        // mkcoin: check genesis block
        {
            CValidationState state;
        ...
        // mkcoin: initialize synchronized checkpoint
        if (!WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // mkcoin: if checkpoint master key changed must reset sync-checkpoint
...
string GetWarnings(string strFor)
{
    ...
    // mkcoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // mkcoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // mkcoin: safe mode for high alert
            }
        }
    }
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            // mkcoin: relay memory may contain blocks too
            bool found = false;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // mkcoin: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // mkcoin: record my external IP reported by peer
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // mkcoin: tell downloading node about the latest block if it's
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // mkcoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // mkcoin: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("MkwaycoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("MkwaycoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MkwaycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MkwaycoinMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    RenameThread(fProofOfStake? "mkcoin-stake-minter" : "mkcoin-miner");
    ...
    try { loop {
        ...
        if (fProofOfStake)
        {
            // mkcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running MkwaycoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("MkwaycoinMiner terminated\n");
        throw;
    }
}
...
{
    // mkcoin: mint proof-of-stake blocks in the background
