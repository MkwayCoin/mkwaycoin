...
// mkcoin: optional setting to unlock wallet for block minting only;
...
int64 CWalletTx::GetTxTime() const
{
    ...
    //mkcoin: we still have the timestamp, so use it to avoid confusion
    return nTime;
}
...
// mkcoin: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->nTime > nSpendTime)
                continue;  // mkcoin: timestamp must not exceed spend time
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                // mkcoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // mkcoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
...
// mkcoin: create coin stake transaction
...
// mkcoin: check 'spent' consistency between wallet and txindex
// mkcoin: fix wallet spent state according to txindex
...
// mkcoin: disable transaction (only for coinstake)
